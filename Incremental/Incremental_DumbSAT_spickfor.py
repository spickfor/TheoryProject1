...
def check(Wff, Nvars, Nclauses, Assignment):
    # Start checking from the first variable (1-indexed)
    index = 1
    ...
    while index <= Nvars:
        ...
        for i in range(Nclauses):
            ...
            for Literal in Clause:
                VarValue = Assignment[abs(Literal)]  # Get the current variable's value
                # Check if the clause is satisfied
                if (Literal > 0 and VarValue == 1) or (Literal < 0 and VarValue == 0):
                    clause_satisfied = True
                    break  # Exit the loop if the clause is satisfied
            
            # If a clause is not satisfied, mark the wff as unsatisfiable
            if not clause_satisfied:
                Satisfiable = False
                break  # No need to check further if one clause fails
        
        # If a satisfying assignment was found, return True
        if Satisfiable:
            return True
        
        # Increment the assignment
        # Reset index to the last modified variable if all are toggled
        while True:
            # Toggle the current variable's value
            Assignment[index] = 1 - Assignment[index]
            if Assignment[index] == 1:  # If toggled to 1, break to check satisfiability
                break
            index += 1  # Move to the next variable
            if index > Nvars:  # If exceeded variable count, return False
                return False
        
        # Reset index to check from the first variable after toggling
        index = 1  # Start checking again from the first variable
